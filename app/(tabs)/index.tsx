...
export default function DiscoverScreen() {
  ...
  // Update campaigns when accounts.data changes
  useEffect(() => {
    try {
      if (accounts.data && Array.isArray(accounts.data) && accounts.data.length > 0) {
        console.log(`🔄 React Query data updated: ${accounts.data.length} campaigns available`);
        ...
        const campaignData = accounts.data.map((account: any) => {
          ...
          return {
            ...
            description: account?.account?.description || 'No description',
            image: account?.account?.image || '',
            reward: bnToNumber(account?.account?.reward) / 1000000000, // Convert lamports to SOL
            endTime: bnToNumber(account?.account?.endtime) || Date.now() / 1000,
            participants: 0, // This would come from participants query
            creator: account?.account?.owner?.toString() || 'unknown',
          };
        });
        
        console.log("📊 Setting campaigns from React Query data:", campaignData);
  ...
  }, [accounts.data, showPastCampaigns])
  ...
  const loadCampaigns = async () => {
    try {
      ...
      if (accounts.data && Array.isArray(accounts.data) && accounts.data.length > 0) {
        ...
        console.log("📊 Campaign data processed:", campaignData);
        setAllCampaigns(campaignData)
        filterCampaigns(campaignData, showPastCampaigns)
      } else {
